...
- (void)_endTouch {
    [self _endLongPressTimer];
    [self _removeHighlightAnimated:_fixBugForFadeOnHighlight ? NO : YES];
    _state.trackingTouch = NO;
}
...
{
    if ([_animationDelegate respondsToSelector:@selector(label:actionForLayer:forKey:)]) {
        return [_animationDelegate label:self actionForLayer:layer forKey:event];
    }
    return [super actionForLayer:layer forKey:event];
}
